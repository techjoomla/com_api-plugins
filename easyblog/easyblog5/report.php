...
class EasyblogApiResourceReport extends ApiResource
{
	public function get()
	{
	$this->plugin->setResponse(Text::_( 'PLG_API_EASYBLOG_USE_METHOD_POST' ));
	}
	...
	public function reportBlog()
	{
		...
		if (!$reason) {
		$message= Text::_( 'PLG_API_EASYBLOG_REASON_EMPTY' );
		...
		$state = $report->store();
		if (!$state) {
			$message= Text::_( 'PLG_API_EASYBLOG_CANT_STORE_REPORT' );
			$final_result['message'] = $message;
			$final_result['status'] = false;
			return $final_result;
		}
		...
		$post = EB::post($id);
		$report->notify($post);
		$final_result['message'] = Text::_( 'PLG_API_EASYBLOG_REPORT_LOGGED_SUCCESS' );
